...
class EGCVBlockS1(nn.Module):
    ...
    def reparameterize(self):
        ...
        if self.use_Beta:
            layers.append(self.BetaBlock.reparameterize() if hasattr(self.BetaBlock, 'reparameterize') else self.BetaBlock)
        if self.use_Gamma:
            layers.append(self.GammaBlock.reparameterize() if hasattr(self.GammaBlock, 'reparameterize') else self.GammaBlock)
...
class EGCVMamba(nn.Module):
    def __init__(self, num_classes=100, stem_channels=32,
...
def EGCVMamba_tiny(num_classes=100, drop_path_rate=0.1):
    return EGCVMamba(
        num_classes=num_classes, stem_channels=24,
...
def EGCVMamba_small(num_classes=100, drop_path_rate=0.1):
    return EGCVMamba(
        num_classes=num_classes, stem_channels=32,
...
def EGCVMamba_base(num_classes=100, drop_path_rate=0.1):
    return EGCVMamba(
        num_classes=num_classes, stem_channels=40,
...
def EGCVMamba_large(num_classes=100, drop_path_rate=0.1):
    return EGCVMamba(
        num_classes=num_classes, stem_channels=48,
...
if __name__ == "__main__":
    ...
    print("1. 初始化模型 & 前向测试")
    print("=" * 60)
    model = EGCVMamba_small(num_classes=100)
    model.eval()
    ...
    total_before += count_params(model.classifier, "Classifier")
    ...
    total_after += count_params(model_rep.classifier, "Classifier")
    print(f"{'总计':<12s} | {total_after / 1e6:>6.2f}M")
    import time
    from tqdm import tqdm

    model = EGCVMamba_tiny(num_classes=100)
    model.eval()
    model = model.cuda()
